...
def _focal_loss_mask(preds, gt, mask):
    ...
    for pred in preds:
        ...
        neg_loss = torch.log(1 - neg_pred) * \
            torch.pow(neg_pred, 2) * neg_weights * neg_mask
...
class CornerNet_Saccade_Loss(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        pull_loss = 0
        push_loss = 0
        tl_tags = [_tranpose_and_gather_feat(
            tl_tag, gt_tl_ind) for tl_tag in tl_tags]
        br_tags = [_tranpose_and_gather_feat(
            br_tag, gt_br_ind) for br_tag in br_tags]
        ...
        off_loss = 0
        tl_offs = [_tranpose_and_gather_feat(
            tl_off, gt_tl_ind) for tl_off in tl_offs]
        br_offs = [_tranpose_and_gather_feat(
            br_off, gt_br_ind) for br_off in br_offs]
        ...
        loss = (focal_loss + att_loss + pull_loss
                + push_loss + off_loss) / max(len(tl_heats), 1)

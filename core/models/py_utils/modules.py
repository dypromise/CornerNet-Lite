...
class hg_net(nn.Module):
    ...
    def _train(self, *xs):
        ...
        tl_modules = [tl_mod_(cnv)
                      for tl_mod_, cnv in zip(self.tl_modules, cnvs)]
        br_modules = [br_mod_(cnv)
                      for br_mod_, cnv in zip(self.br_modules, cnvs)]
        tl_heats = [tl_heat_(tl_mod)
                    for tl_heat_, tl_mod in zip(self.tl_heats, tl_modules)]
        br_heats = [br_heat_(br_mod)
                    for br_heat_, br_mod in zip(self.br_heats, br_modules)]
        tl_tags = [tl_tag_(tl_mod)
                   for tl_tag_, tl_mod in zip(self.tl_tags, tl_modules)]
        br_tags = [br_tag_(br_mod)
                   for br_tag_, br_mod in zip(self.br_tags, br_modules)]
        tl_offs = [tl_off_(tl_mod)
                   for tl_off_, tl_mod in zip(self.tl_offs, tl_modules)]
        br_offs = [br_off_(br_mod)
...
class saccade_net(nn.Module):
    ...
    def _train(self, *xs):
        ...
        br_offs = [br_off_(br_mod)
                   for br_off_, br_mod in zip(self.br_offs, br_modules)]
        atts = [[att_mod_(u) for att_mod_, u in zip(att_mods, up)]
    ...
    def _test(self, *xs, no_att=False, **kwargs):
        ...
        if not no_att:
            atts = [att_mod_(up) for att_mod_, up in zip(
                self.att_modules[-1], ups[-1])]
